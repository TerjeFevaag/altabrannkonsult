...
const MAX_FILES = 4
const BLOB_HOSTNAME_PATTERN = /\.public\.blob\.vercel-storage\.com$/
...
function isValidVedlegg(value: unknown): value is Vedlegg {
...
export async function POST(request: Request) {
  ...
  const rawVedlegg = Array.isArray(body.vedlegg) ? body.vedlegg : []
  if (rawVedlegg.length > MAX_FILES) {
    return NextResponse.json({ error: `Du kan laste opp maks ${MAX_FILES} filer.` }, { status: 400 })
  }
  if (!rawVedlegg.every(isValidVedlegg)) {
    return NextResponse.json({ error: 'Et eller flere vedlegg er ugyldige. Prøv å laste opp filene på nytt.' }, { status: 400 })
  }
  const vedlegg = rawVedlegg as Vedlegg[]
  ...
  const textBody = [
    ...
    vedlegg.length > 0
      ? `Vedlegg:\n${vedlegg.map((file) => `- ${file.name}: ${file.url}`).join('\n')}`
      : 'Vedlegg: Ingen',
    '',
    'Melding:',
    melding,
  ...
  const htmlBody = `
    ...
    <p><strong>Vedlegg:</strong>${
      vedlegg.length > 0
        ? '<br />' + vedlegg.map((file) => `<a href="${escapeHtml(file.url)}">${escapeHtml(file.name)}</a>`).join('<br />')
        : ' Ingen'
    }</p>
